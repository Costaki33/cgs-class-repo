...
def hemisphere_summary_list(ne, nw, se, sw):
...
def main():
    ...
    for row in ml_data['meteorite_landings']:
        ...
        if (location == 'Northern & Western'):
            nw += 1

        if (location == 'Southern & Eastern'):
            se += 1

        if (location == 'Southern & Western'):
            sw += 1

    hemisphere_summary_list(ne, nw, se, sw)
